<?php
	include "../db_proses/koneksi.php";
  $id=$_POST['id'];
  $query = "SELECT * FROM tb_produk INNER JOIN tb_kategori ON tb_kategori.id_kategori=tb_produk.kategori_produk WHERE tb_produk.id_produk='$id'";
	$result = mysqli_query($kon, $query);
	
	while($baris = mysqli_fetch_assoc($result)){
?>

                  <form method="post" id="form-edit">
                    <div class="form-group" style="padding-bottom: 20px;">
                      <label for="kode">Kode Produk*</label>
                      <input class="form-control" type="text" name="kode_produk" readonly id="kode_produk" value="<?php echo $baris['id_produk']; ?>" required/>  
                    </div>
                    <div class="form-group" style="padding-bottom: 20px;">
                      <label for="nama">Nama Produk*</label>
                      <input class="form-control" type="text" name="nama_produk" readonly id="nama_produk" value="<?php echo $baris['nama_produk']; ?>" required/>
                    </div>
                    <div class="form-group" style="padding-bottom: 20px;">
                      <label for="region">Urutan Produk*</label>
                      <select class="form-control" name="seq" id="seq">
                        <option value="">Pilih Urutan Produk</option>
    <?php                      
      for($a=1; $a <= 10; $a++)
        {
    ?>
                      <option value="<?php echo $a; ?>"><?php echo $a; ?></option>
    <?php
        }
    ?>
                      </select>
                    </div>
                    <div class="form-group" style="padding-bottom: 20px;">
                      <label for="region">Produk*</label>
                      <select class="form-control" name="pdk" id="pdk">
                        <option value="">Pilih Produk</option>
    <?php
      $query1 = "SELECT * FROM tb_produk WHERE kategori_produk != 'PKT'";
      $result1 = mysqli_query($kon, $query1);
      
      while($baris1 = mysqli_fetch_assoc($result1)){
    ?>
                      <option value="<?php echo $baris1['id_produk']; ?>"><?php echo $baris1['id_produk']." - ".$baris1['nama_produk']; ?></option>
    <?php
        }
    ?>
                      </select>
                    </div>                    
                    <div class="form-group" style="padding-bottom: 20px;">
                      <label for="nama">Jumlah Produk*</label>
                      <input class="form-control" type="text" name="jum_produk" id="jum_produk" required/>
                    </div>
                  </form>

<?php
  }
?>