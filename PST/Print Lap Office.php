...
?>
  <head>
    ...
    <title> Laporan Office </title>
  </head>

            <div>
              Laporan BTB Office <?php echo $_COOKIE['office_bill']; ?> <br>
    ...
    <?php
      if($model=="REKAP"){
        ...
        if ($pdk=="ALL"){
          $query="SELECT DISTINCT tb_produk.id_produk, tb_produk.nama_produk FROM tb_produk INNER JOIN tb_detail_beli ON tb_detail_beli.produk_detbeli=produk.id_produk 
    ...
    <?php
                  ...
                  if ($nota=="ALL"){
                    ...
                    INNER JOIN tb_detail_beli ON tb_detail_beli.produk_detbeli=tb_produk.id_produk 
                    INNER JOIN tb_beli ON tb_beli.id_beli=tb_detail_beli.nota_detbeli
                    WHERE tb_tb_beli.kantor_beli='$kantor' AND tb_detail_beli.produk_detbeli='$pdk' AND tb_beli.tgl_beli BETWEEN '$tgl_mulai' AND '$tgl_selesai'";
                  }else{
                    $query1="SELECT * FROM tb_produk 
                    INNER JOIN tb_detail_beli ON tb_detail_beli.produk_detbeli=tb_produk.id_produk 
                    INNER JOIN tb_beli ON tb_beli.id_beli=tb_detail_beli.nota_detbeli
