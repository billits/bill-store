<?php
  ...
  $sqlre= "SELECT * FROM tb_beli INNER JOIN tb_staff ON tb_staff.kode_staff=tb_beli.counter_beli 
  INNER JOIN tb_office ON tb_office.id_office=tb_staff.office_staff WHERE tb_beli.id_beli='$nota'";
	$resultre = mysqli_query($kon,$sqlre);
  $rowre = mysqli_fetch_array($resultre,MYSQLI_ASSOC);
  $supply = $rowre['supplier_beli'];
...
<html>
  ...
  <font face="verdana" size="-2">
    <p>
        ...
        <tr>
          <td>
            <table>
              ...
              <tr><td>Tanggal</td><td>:</td><td><?php echo date("d-m-Y H:i:s", strtotime($rowre['tgl_beli'])); ?></td>
              <tr><td>Cabang</td><td>:</td><td><?php echo $rowre['nama_staff']." - ".$rowre['nama_office']; ?></td>
            </table>
          </td>
          <td>
            <table>
              <tr><td>Supplier</td><td>:</td><td><?php echo $row['nama_staff']." - ".$row['nama_office']; ?></td>
              <tr><td>Keterangan</td><td>:</td><td><?php echo $rowre['keterangan_beli']; ?></td>
              <tr><td>Status PO</td><td>:</td><td><?php echo $rowre['status_beli']; ?></td>
            </table>
          </td>
        </tr>
      </table>
      ...
      Tanggal : ".date("d-m-Y H:i:s", strtotime($rowre['tgl_beli']))."<br>
      Supplier  : ".$row['nama_staff']." - ".$row['nama_office']."<br>
      Keterangan : ".$rowre['keterangan_beli']; ?> -->
    ...
    <?php
      ...
      $bayar=0;
      $query2="SELECT * FROM tb_detail_beli INNER JOIN tb_beli ON tb_beli.id_beli=tb_detail_beli.nota_detbeli 
      ...
      while($baris2 = mysqli_fetch_assoc($result2)){
        $bayar=$bayar+$baris2['total_jumlah_detbeli'];
        $produk=$baris2['id_produk'];
        $totju=$baris2['total_beli'];
        
    ?>
      <tr>
        <td align="center" style="border:1px solid black; padding:5px; border-collapse: collapse;"><?php echo $baris2['qty_detbeli']; ?></td>
        ...
        <td style="border:1px solid black; padding:5px; border-collapse: collapse;"><?php echo rupiah($baris2['total_jumlah_detbeli']); ?></td>
      </tr>
    <?php
      }
    ?>  
      <tr>
        ...
        <td style="border:1px solid black; padding:5px; border-collapse: collapse;"> <?php echo rupiah($rowre['total_beli']); ?> </td>
      </tr>
    </table>
    <br>
