<?php
  ...
  if (empty($staff)||empty($nota)||empty($produk)||empty($price)||empty($qty)||empty($hh_pro)||empty($office)||empty($kategori)){
	...
	$sqlre1= "SELECT * FROM tb_detail_mankonsi WHERE nota_detmk='$nota' AND produk_detmk='$produk'";
  ...
  if($countre1 >= 1) {  
	...
	if ($valid==0){
		...
		$sql = "INSERT INTO tb_detail_mankonsi(nota_detmk, produk_detmk, harga_detmk, qty_detmk, diskon_detmk, totjum_detmk) VALUES 
