<?php
  ...
  if (empty($nota)||empty($evt)||empty($staff)||empty($ketsup)){
    ...
    while($baris = mysqli_fetch_assoc($result)){
      ...
      tgl_update_gudang=NOW() WHERE kode_produk_gudang='$det_pdk' AND kode_office_gudang='PST' AND event_gudang='OFFICE'";
    ...
    $sqlll = "INSERT INTO tb_notif_retur(event_retur, status_retur, waktu_retur, staff_retur) VALUES ('$evt', '1', NOW(), '$staff')";
    ...
    if ($cek==0){
      ...
      $status['nott']=$nota; 
      $status['error']="Data Berhasil Diproses";	
    }else{          
      mysqli_rollback($kon);
