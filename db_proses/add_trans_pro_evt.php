<?php
  ...
  if (empty($staff)||empty($nota)||empty($produk)||empty($price)||empty($qty)||empty($hh_pro)||empty($office)||empty($kategori)||empty($event)){
		$valid=1;
	}
  ...
  if($countre1 < 1) {
			...
			$sqlre= "SELECT * FROM tb_gudang WHERE kode_office_gudang='$office' AND kode_produk_gudang='$produk' AND event_gudang='$event'";
	...
	if ($valid==0){
		...
		if ($act==2){
			$sql = "INSERT INTO tb_detail_jual(nota_detjual, produk_detjual, harga_detjual, qty_detjual, diskon_detjual, total_jumlah_detjual) VALUES 
