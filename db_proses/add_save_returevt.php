<?php
  ...
  if (empty($ketsup)||empty($nota)||empty($total)||empty($evnt)||empty($staff)){
	...
	if ($valid==0){
		...
		if ($cont!=0){
			//gakusah insert notif retur krena belum selesai retur semua
			...
			while($data=mysqli_fetch_array($ss)){
				...
				// $sli1 = "UPDATE gudang SET jml_produk_gudang=jml_produk_gudang-'$jum', supplier_gudang='$staff', staff_gudang='$staff', 
				// tgl_update_gudang=NOW() WHERE kode_produk_gudang='$pro' AND event_gudang='$evnt'";
		...
		}else {//insert notif retur krena sudah selesai retur semua
			$sql="SELECT * FROM tb_detail_retur_event WHERE id_detretevt='$nota'";
			$ss=mysqli_query($kon,$sql);
			while($data=mysqli_fetch_array($ss)){
				$pro=$data['produk_detretevt'];
