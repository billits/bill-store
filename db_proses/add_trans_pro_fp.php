<?php
  ...
  if (empty($staff)||empty($nota)||empty($produk)||empty($price)||empty($qty)||empty($hh_pro)||empty($office)||empty($kategori)){
	...
	$sqlre1= "SELECT * FROM tb_detail_fp WHERE id_detfp='$nota' AND produk_detfp='$produk'";
  ...
  if($countre1 < 1) {  	
	...
	if ($valid==0){
		...
		$sql = "INSERT INTO tb_detail_fp(id_detfp, produk_detfp, harga_detfp, jum_detfp, totjum_detfp, diskon_detfp) VALUES 
			('$nota', '$produk', '$hh_pro', '$qty', '$tot', '$diskon')";
