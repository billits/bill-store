<?php
  ...
  if (empty($total)||empty($nota)||empty($ketsup)||empty($kantor)||empty($staff)){
	...
	$sql_tmp= "SELECT * FROM tb_beli WHERE id_beli= '$nota'";
  $result_tmp = mysqli_query($kon,$sql_tmp);
  $row_tmp = mysqli_fetch_array($result_tmp,MYSQLI_ASSOC);
	$waktu = $row_tmp['tgl_beli'];

	if ($valid==0){
		mysqli_autocommit($kon, false);

		$sl = "UPDATE tb_beli SET total_beli='$total', supplier_beli='$staff', tgl_beli='$waktu', tgl_approv_beli=NOW(), status_beli='APPROVE', counter_beli='$staff', keterangan_beli='$ketsup' 
						WHERE id_beli='$nota' AND status_beli='PENDING' AND event_beli='OFFICE'";
