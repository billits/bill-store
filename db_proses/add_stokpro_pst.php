<?php
  ...
  if (empty($nota)||empty($produk)||empty($price)||empty($qty)||empty($hh_pro)){
	...
	$sqlre= "SELECT * FROM tb_detail_beli WHERE nota_detbeli= '$nota' AND produk_detbeli='$produk'";
	...
	if ($valid==0){
		...
		$sql = "INSERT INTO tb_detail_beli(nota_detbeli, produk_detbeli, harga_detbeli, qty_detbeli, diskon_detbeli, total_jumlah_detbeli) 
