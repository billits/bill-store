<?php
  ...
  $sqlre= "SELECT * FROM tb_retur_transaksi 
  INNER JOIN tb_staff ON tb_staff.kode_staff=tb_retur_transaksi.staff_retur 
...
<html>
  ...
  <font face="verdana" size="-2">
        ...
        <tr>
          <td>
            <table>
              ...
              <tr><td>Tanggal</td><td>:</td><td><?php echo date("d-m-Y H:i:s", strtotime($rowre['waktu_retur'])); ?></td>
              <tr><td>Jenis</td><td>:</td><td><?php echo $rowre['jenis_retur']; ?></td>
            </table>
          </td>
          <td>
            <table>
              <tr><td>Dari Nota</td><td>:</td><td><?php echo $rowre['notajual_retur']; ?></td>
              <tr><td>Staff</td><td>:</td><td><?php echo $rowre['nama_staff']; ?></td>
              <tr><td>Keterangan</td><td>:</td><td><?php echo $rowre['keterangan_retur']; ?></td>
            </table>
          </td>
        </tr>
      </table>
    ...
    <?php
      ...
      $satuan=0;
      $bayar=0;
      $query2="SELECT * FROM tb_detail_retur_transaksi 
      ...
      while($baris2 = mysqli_fetch_assoc($result2)){
        $satuan=$baris2['totjum_detrettrans']/$baris2['jumret_detrettrans'];
    ...
    ?>
      <tr>
        ...
        <td align="center" style="border:1px solid black; padding:5px; border-collapse: collapse;"><?php echo $baris2['jumret_detrettrans']; ?></td>
        <td style="border:1px solid black; padding:5px; border-collapse: collapse;"><?php echo rupiah($satuan); ?></td>
        <td style="border:1px solid black; padding:5px; border-collapse: collapse;"><?php echo rupiah($baris2['totjum_detrettrans']); ?></td>
      </tr>
    <?php
      }
    ?>  
